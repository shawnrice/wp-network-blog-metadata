...
function nbm_update_sql_table() {
	...
	$t = count( $wpdb->get_results( "show columns from `$tablename` like 'class_type'" ) );
	if ( ! $t ) {
		$sql = ( "ALTER TABLE `$tablename` ADD `class_type` VARCHAR(45) NULL" );
		$wpdb->get_results( $sql );
	}
	$t = count( $wpdb->get_results( "show columns from `$tablename` like 'class_name'" ) );
	if ( ! $t ) {
		$sql = ( "ALTER TABLE `$tablename` CHANGE COLUMN `course_name` `class_name`" );
		$wpdb->get_results( $sql );
	}
	$t = count( $wpdb->get_results( "show columns from `$tablename` like 'class_number'" ) );
	if ( ! $t ) {
		$sql = ( "ALTER TABLE `$tablename` CHANGE COLUMN `course_number` `class_number`" );
		$wpdb->get_results( $sql );
	}
