...
function nbm_download_csv_blogs_meta() {
	...
	$sql = "SELECT $tablename.*, wp_blogs.registered FROM wp_blogs
	LEFT OUTER JOIN $tablename ON $tablename.blog_id=wp_blogs.blog_id";
...
function nbm_network_admin_export_tab() {
	...
	$sql = "SELECT $tablename.*, wp_blogs.registered FROM wp_blogs
	LEFT OUTER JOIN $tablename ON $tablename.blog_id=wp_blogs.blog_id LIMIT 0, 50";

	$data = $wpdb->get_results( $sql ,  ARRAY_A );    // Selects all the roles in the wpnbm_data table
...
blog_id,role,purpose,class_name,class_number,major,department,program,class_type,registered
<?php
	foreach ( $data as $datum ) {
